# V2: loss + grad_norm_pre_allreduce logging (DDP comm hook) + jsonl per rank
...
def make_comm_hook(gn: PreAllReduceGradNorm):
    def hook(state, bucket):
        ...
        # 新的一次 backward 同步：bucket index=0 视为开始（best-effort）
        if hasattr(bucket, "get_index") and callable(bucket.get_index) and bucket.get_index() == 0:
            gn.reset()
...
def main():
    ...
    output_path = "/home/dndx/lihong/op项目/outputs"
